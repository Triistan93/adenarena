// ========================================
// Items Database - Lineage Idle (CORRIGIDO)
// ========================================
//
// CORREÇÕES APLICADAS (ver README de correções no App):
//  1. ORDEM DE DECLARAÇÃO: ALL_ITEMS agora é montado DEPOIS de todas as
//     constantes (LEGS, SHIELDS, NECKLACES, ...). No original, ALL_ITEMS
//     usava `...LEGS` etc. ANTES dessas constantes existirem -> ReferenceError
//     (Temporal Dead Zone) que quebrava o arquivo inteiro.
//  2. ICON_MAP: removidas as chaves-lixo com dupla extensão (".png.png").
//  3. Ícones inline padronizados SEM extensão (imperial_crusader_breastplate).
//  4. window.GameData agora exporta TODAS as constantes.
//  5. Helpers com guarda contra id inexistente.
//  6. Suporte a ambiente sem `window` (ESM export) para uso em React/Vite.
...
const SLOT = {
  ...
  legs: 'Legs', shield: 'Shield', necklace: 'Necklace', earring: 'Earring', belt: 'Belt',
  cloak: 'Cloak', talisman: 'Talisman', hair: 'Hair', hair2: 'Mask', agathion: 'Agathion',
  powerup: 'Powerup'
};
...
const ARMORS = {
  ...
  imperial_crusader_breastplate: { name: 'Imperial Crusader Breastplate', slot: 'armor', tier: 5, def: 220, mdef: 120, hp: 650, req: { level: 76 }, price: 45000, icon: 'imperial_crusader_breastplate', desc: 'Peitoral da lendária armadura pesada Imperial Crusader de Grau S.' },
...
// Nota: itens de anel abaixo estavam originalmente misturados em EARRINGS.
// Mantidos como no original para não quebrar referências de loot (ring_queen_ant / ring_baium).
const EARRINGS = {
  ...
  ring_queen_ant: { name: 'Ring of Queen Ant', slot: 'ring', tier: 5, crit: 15, atk: 50, req: { level: 40 }, price: 50000, icon: 'accessory_ring_of_queen_ant_i03', desc: 'Anel épico da Rainha Formiga. (+15% Crit Rate, +50 ATK)' },
...
const BELTS = {
  novice_belt: { name: 'Novice Leather Belt', slot: 'belt', tier: 1, def: 2, hp: 15, req: { level: 1 }, price: 60, icon: 'belt' },
  ...
  champion_belt: { name: 'Champion Belt', slot: 'belt', tier: 4, def: 35, atk: 20, hp: 200, req: { level: 60 }, price: 15000, icon: 'blessed_belt' },
  dragon_belt: { name: 'Dragon Belt', slot: 'belt', tier: 5, def: 55, atk: 40, matk: 40, hp: 350, req: { level: 76 }, price: 30000 }
};

const CLOAKS = {
  adventurer_cloak: { name: 'Adventurer Cloak', slot: 'cloak', tier: 1, def: 2, mdef: 2, speed: 3, req: { level: 1 }, price: 100, icon: 'capared' },
  cloak_of_valor: { name: 'Cloak of Valor', slot: 'cloak', tier: 2, def: 6, mdef: 8, speed: 5, req: { level: 20 }, price: 1200, icon: 'capawhite' },
...
// ======================================
// ALL_ITEMS — montado DEPOIS de TODAS as constantes (corrige TDZ)
// ======================================
const ALL_ITEMS = {
  ...
  ...NEW_ARMORS, ...CLASS_WEAPONS, ...CLASS_ARMORS, ...POWERUPS
};

// Aliases para garantir que lookups nunca retornem undefined
...
// Explicit PNG Icon File Mappings (icons SEM extensão, consistente com ICON_MAP)
...
// ======================================
const MONSTER_DROPS = {
  goblin: { items: [{ id: 'iron_ore', chance: 0.15, amount: [1, 2] }, { id: 'cloth', chance: 0.15, amount: [1, 2] }, { id: 'goblin_ear', chance: 0.2, amount: [1, 1] }, { id: 'hp_potion_s', chance: 0.05, amount: [1, 1] }], equipment: [{ pool: ['wooden_sword', 'training_dagger', 'oak_staff', 'short_bow', 'leather_vest', 'cloth_robe', 'cloth_pants', 'wooden_shield', 'leather_helm', 'cloth_boots', 'leather_gloves', 'oak_necklace', 'oak_earring', 'copper_ring', 'novice_belt', 'adventurer_cloak', 'talisman_novice', 'apprentice_circlet', 'novice_mask'], chance: 0.10 }] },
  wolf: { items: [{ id: 'beast_blood', chance: 0.15, amount: [1, 2] }, { id: 'wolf_fang', chance: 0.15, amount: [1, 2] }, { id: 'leather', chance: 0.15, amount: [1, 2] }], equipment: [{ pool: ['training_dagger', 'short_bow', 'leather_vest', 'leather_gaiters', 'leather_helm', 'leather_boots', 'leather_gloves', 'oak_necklace', 'oak_earring', 'copper_ring', 'novice_belt', 'adventurer_cloak', 'talisman_novice'], chance: 0.10 }] },
  spider: { items: [{ id: 'beast_blood', chance: 0.15, amount: [1, 2] }, { id: 'magic_powder', chance: 0.15, amount: [1, 1] }, { id: 'cloth', chance: 0.15, amount: [1, 2] }], equipment: [{ pool: ['oak_staff', 'cloth_robe', 'cloth_pants', 'cloth_boots', 'oak_necklace', 'oak_earring', 'copper_ring', 'apprentice_circlet', 'talisman_novice'], chance: 0.10 }] },
  orc: { items: [{ id: 'iron_ore', chance: 0.2, amount: [2, 4] }, { id: 'leather', chance: 0.15, amount: [1, 3] }, { id: 'hp_potion_m', chance: 0.05, amount: [1, 1] }], equipment: [{ pool: ['iron_sword', 'bronze_mace', 'iron_gaiters', 'bronze_shield', 'iron_boots', 'iron_gauntlets', 'iron_helm', 'silver_ring', 'elven_necklace', 'elven_earring', 'mithril_belt', 'cloak_of_valor', 'talisman_protection'], chance: 0.12 }] },
  kobold: { items: [{ id: 'iron_ore', chance: 0.2, amount: [1, 3] }, { id: 'steel_ingot', chance: 0.1, amount: [1, 1] }, { id: 'mp_potion_s', chance: 0.05, amount: [1, 1] }], equipment: [{ pool: ['iron_sword', 'steel_dagger', 'composite_bow', 'iron_armor', 'iron_gaiters', 'bronze_shield', 'iron_boots', 'iron_helm', 'silver_ring', 'mithril_belt'], chance: 0.12 }] },
  kamaelScout: { items: [{ id: 'magic_powder', chance: 0.15, amount: [1, 3] }, { id: 'crystal_fragment', chance: 0.15, amount: [1, 2] }], equipment: [{ pool: ['crystal_staff', 'assassins_blade', 'mage_stockings', 'sigil_of_devotion', 'cloak_of_valor', 'assassin_mask', 'shadow_boots', 'silver_ring', 'elven_earring'], chance: 0.12 }] },
  skeleton: { items: [{ id: 'iron_ore', chance: 0.2, amount: [1, 2] }, { id: 'steel_ingot', chance: 0.1, amount: [1, 1] }], equipment: [{ pool: ['iron_sword', 'crystal_staff', 'iron_armor', 'mage_robe', 'mage_stockings', 'iron_gaiters', 'bronze_shield', 'mage_hood', 'iron_helm', 'talisman_protection'], chance: 0.12 }] },
  goblinKing: { items: [{ id: 'steel_ingot', chance: 0.3, amount: [2, 4] }, { id: 'mithril_ore', chance: 0.15, amount: [1, 2] }, { id: 'scroll_of_resurrection', chance: 0.2, amount: [1, 1] }], equipment: [{ pool: ['knight_sword', 'archmage_staff', 'elven_bow', 'warhammer', 'dark_katana', 'steel_plate', 'steel_gaiters', 'steel_shield', 'sigil_of_grace', 'steel_helm', 'steel_boots', 'gold_ring', 'necklace_of_grace', 'earring_of_grace', 'belt_of_power', 'cloak_of_freedom', 'talisman_of_power', 'silver_tiara'], chance: 0.22, rarityBoost: 1 }] },
  wolfAlpha: { items: [{ id: 'wolf_fang', chance: 0.3, amount: [3, 6] }, { id: 'beast_blood', chance: 0.3, amount: [2, 4] }, { id: 'leather', chance: 0.3, amount: [3, 5] }], equipment: [{ pool: ['steel_dagger', 'composite_bow', 'iron_armor', 'shadow_pants', 'cloak_of_valor', 'steel_boots', 'silver_ring', 'mithril_belt'], chance: 0.20 }] },
  knight: { items: [{ id: 'steel_ingot', chance: 0.2, amount: [2, 4] }, { id: 'mithril_ore', chance: 0.15, amount: [1, 2] }], equipment: [{ pool: ['blade_of_doom', 'soul_seeker', 'staff_of_magic', 'dragon_bow', 'titan_hammer', 'dual_swords', 'knight_armor', 'arcane_robe', 'knight_gaiters', 'arcane_stockings', 'knight_shield', 'sigil_of_mastery', 'knight_helm', 'arcane_circlet', 'diamond_ring', 'onyx_ring', 'amethyst_ring', 'black_ore_necklace', 'black_ore_earring', 'champion_belt', 'cloak_of_shadows_gear', 'talisman_of_eva', 'noble_gold_crown', 'executioner_mask'], chance: 0.15, rarityBoost: 1 }] },
  mage: { items: [{ id: 'magic_powder', chance: 0.2, amount: [2, 4] }, { id: 'crystal_fragment', chance: 0.2, amount: [2, 3] }, { id: 'mithril_ore', chance: 0.1, amount: [1, 2] }], equipment: [{ pool: ['archmage_staff', 'staff_of_magic', 'arcane_robe', 'arcane_stockings', 'sigil_of_mastery', 'arcane_circlet', 'arcane_boots', 'arcane_gloves', 'ruby_ring', 'sapphire_ring', 'black_ore_necklace', 'black_ore_earring', 'talisman_of_eva'], chance: 0.15, rarityBoost: 1 }] },
...
// ======================================
// ROLETA DE RARIDADE E DROPS
// ======================================
function rollRarity(bonus = 0) {
  const r = Math.random();
  if (r < 0.005 + bonus * 0.005) return 'legendary';
  if (r < 0.045 + bonus * 0.01) return 'epic';
  if (r < 0.15 + bonus * 0.02) return 'rare';
  if (r < 0.45 + bonus * 0.03) return 'uncommon';
  return 'common';
}
...
function rollDrop(target, lootBonus = 1) {
  ...
  for (const equipmentDrop of monsterDrops.equipment || []) {
    ...
    const cappedChance = Math.min(0.03, rawChance) * chanceMultiplier;
...
// Guarda contra id inexistente para não lançar TypeError.
function rollItemWithRarity(itemId, rarity) {
  const source = ALL_ITEMS[itemId];
  if (!source) {
    console.warn(`[rollItemWithRarity] item desconhecido: ${itemId}`);
    return null;
  }
  const base = JSON.parse(JSON.stringify(source));
  base.rarity = RARITY[rarity] ? rarity : 'common';
...
// ICON_MAP limpo (removidas as chaves-lixo com dupla extensão)
const ICON_MAP = {
...
// Retorna o caminho do ícone com fallback seguro.
...
const GameData = {
  ICON_MAP, RARITY, SLOT,
  WEAPONS, ARMORS, LEGS, HELMETS, BOOTS, GLOVES, SHIELDS,
  NECKLACES, EARRINGS, BELTS, CLOAKS, TALISMANS, HAIR, HAIR2,
  RINGS, CONSUMABLES, MATERIALS, AGATHIONS, POWERUPS,
  CLASS_WEAPONS, CLASS_ARMORS, NEW_ARMORS,
  ALL_ITEMS, MONSTER_DROPS, SHOP_INVENTORY, CRAFTING_RECIPES,
  ZONE_GOLD_MULT, MYSTIC_POOL,
  rollRarity, rollDrop, getMysticRotation, rollItemWithRarity, getIconPath
};

// Compat: também expõe em window quando disponível (mantém o comportamento original)
