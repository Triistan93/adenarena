...
const RACES_INFO: Record<string, {
  ...
  human: {
    ...
    name: 'Humano',
    icon: '­ƒºæÔÇì­ƒî¥',
    desc: 'Vers├íteis e equilibrados em todas as disciplinas de combate e magia.',
...
export const CharacterCreation: React.FC<CharacterCreationProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const finalName = charName.trim() || 'Her├│i de Aden';
    onComplete({
      charName: finalName,
      race: selectedRace,
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs space-y-1">
              <span className="font-bold text-amber-300 block mb-1">Ô£¿ B├┤nus Raciais de {currentRaceObj.name}:</span>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between rounded-xl border border-white/10 bg-white/[0.02] p-4 text-center">
            ...
            <div className="mt-6 space-y-2">
              ...
              >
                Ô£¿ Criar Personagem & Entrar em Aden
              </button>
              {onCancel && (
                <button
                  type="button"
