...
export function LoginScreen({ onEnterGame }: LoginScreenProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 overflow-y-auto text-white flex items-start sm:items-center justify-center p-3 sm:p-6 min-h-[100dvh] bg-cover bg-center bg-no-repeat select-none"
      style={{ backgroundImage: "url('/img/login/aden_login_clean_bg.jpg')" }}
    >
      {/* Vinheta e Blur Suave no Fundo */}
      ...
      {/* Painel de Login Original (Transparente e Elegante com Bordas Douradas) */}
      <div className="relative w-full max-w-md my-auto bg-[#0b0f1c]/90 border border-amber-500/30 rounded-3xl p-5 sm:p-8 shadow-2xl backdrop-blur-xl">
        {/* Game Title Header */}
        ...
        {user ? (
          <div className="space-y-4">
            <div className="text-center p-4 bg-white/5 border border-white/10 rounded-2xl">
              ...
              <h2 className="font-display font-bold text-lg text-amber-300 mt-1">Bem-vindo de volta!</h2>
              <p className="text-xs text-white/60 truncate">{user.email}</p>

              {cloudState ? (
                <div className="mt-3 p-3 bg-black/40 rounded-xl border border-white/5 text-left text-xs space-y-1.5">
