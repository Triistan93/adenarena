...
console.log(`Unmapped icons count: ${unmapped.length}`);
...
function determineSlotAndStats(id, path, gl) {
  ...
  if (i.includes('helmet') || i.includes('helm') || i.includes('cap') || i.includes('hood') || i.includes('circlet') || i.includes('hat') || i.includes('goggle') || i.includes('mask') || i.includes('crown') || i.includes('ears')) {
    const isAcc = i.includes('hat') || i.includes('goggle') || i.includes('mask') || i.includes('crown') || i.includes('ears') || i.includes('circlet');
  ...
  if (i.includes('cloak') || i.includes('capa') || i.includes('capa')) {
    return {
      slot: 'cloak',
  ...
  if (p.includes('materials') || i.includes('ore') || i.includes('pouch') || i.includes('suede') || i.includes('leather') || i.includes('fiber') || i.includes('coal') || i.includes('gem') || i.includes('bone') || i.includes('lump') || i.includes('oil')) {
    return {
      slot: 'material',
  ...
  return {
    slot: 'consumable',
    def: { name, slot: 'consumable', stack: 99, price: Math.floor(lvl * 20 + 50), desc: `Item consumível especial (${name}).` }
  };
}
...
console.log(`Generated definitions for ${Object.keys(generatedDefs).length} unmapped items!`);
console.log('\nSample generated definitions:');
const keys = Object.keys(generatedDefs);
keys.slice(0, 10).forEach(k => console.log(`  ${k}:`, generatedDefs[k]));
